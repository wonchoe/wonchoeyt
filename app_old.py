...
# Lock only if not in test mode (no args)
if len(sys.argv) == 1:
    lock_or_exit()
...
async def download(
    ...
    def sync_download():
        ...
        if mode == AUDIO:
            ...
            if video_fmt:
                opts["format"] = f"bestvideo[height<={video_fmt}][ext=mp4]+bestaudio[ext=m4a]/best[height<={video_fmt}]"
            else:
...
# TEST MODE HANDLER
# ---------------------------------------------------------
async def test_download(url: str, mode: str = AUDIO, quality: str = "720"):
    """Тестовий режим для завантаження без бота"""
    log.info(f"🧪 TEST MODE: {mode} from {url}")
        ...
        if d["status"] == "downloading":
            ...
            if total > 0:
                ...
                if time.time() - last_update[0] > 0.5:
                    last_update[0] = time.time()
                    bar = make_bar(percent)
                    log.info(f"⬇️  {bar} {percent:.1f}%")
            else:
                if time.time() - last_update[0] > 0.5:
                    last_update[0] = time.time()
                    mb = done / 1024 / 1024
                    log.info(f"⬇️  {mb:.1f} MB")
    ...
    try:
        ...
        file_size = fp.stat().st_size
        log.info(f"✅ Downloaded: {fp.name} ({file_size / 1024 / 1024:.1f} MB)")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        asyncio.run(test_download(url, mode, quality))
        return
